...
from src.interpolants import linear_interpolant_values, bspline_interpolant_values, hermite_interpolant_values

# Repo root, resolved relative to this file (src/helpers.py -> repo root)
PROJECT_ROOT = Path(__file__).resolve().parent.parent


def get_model(model_config, data_dim, input_dim, device, dynamics_kind='ode'):
    model_config = json.load(open(PROJECT_ROOT / "configs" / "model_configs" / f"{model_config}.json"))
...
def get_flow_matching_inputs(values, times, mask, interpolant_kind="linear", degree=3, subsample_per_interval=32, dynamics_kind='ode', sigma=0.001, pairs=None, unpaired=None):
