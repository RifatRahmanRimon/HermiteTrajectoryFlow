...
def parse_pairs(pairs_str):
    """Parse a '--pairs' string like '0:1,2:3' into [(0, 1), (2, 3)]."""
...
def main() -> None:
    ...
    train_loader_unprocessed, test_loader_unprocessed = get_dataloaders(args.data_config, batch_size=args.batch_size, data_sample_complexity= args.data_sample_complexity, mask_mode=args.mask_mode)
    ...
    train_loader= ProcessedDataset(train_loader_unprocessed, train=True, interpolant_kind= args.interpolant_kind, degree=args.degree, subsample_per_interval= args.subsample_per_interval, device=device, dynamics_kind=args.dynamics_kind, sigma=sigma, pairs=pairs)
    test_loader= ProcessedDataset(test_loader_unprocessed, interpolant_kind= args.interpolant_kind, degree=args.degree, subsample_per_interval= args.subsample_per_interval, device=device, stats= train_loader.stats, dynamics_kind=args.dynamics_kind, sigma=train_loader.sigma, pairs=pairs)
